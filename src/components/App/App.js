...
export default function App(){
    ...
    const [metaPoints, setMetaPoints] = React.useState(1)
    ...
    function meta(event){
        setMetaPoints(event.target.value)
        console.log(metaPoints, "meta")
    ...
    function init(){
        if(card === "React JSX"){
            setText(reactJSX)
            setMaxValue(reactJSX.length)

        }else if(card === "XGH"){
            setText(XGH)
            setMaxValue(XGH.length)
        }
        if(start === "home" && (card === "React JSX" || card === "XGH") && metaPoints <= maxValue){
            (setStart("Zap"))
        }else{
            setStart("home")
        }
    ...
    return (
        (start === "home") ? <Container1 className="app"  init={init} deck={deck} meta={meta}card={card} maxValue={maxValue} metaPoints={metaPoints}/> : <Container2  init={init} text={text} metaPoints={metaPoints}/>
    )
}